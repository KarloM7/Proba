 ...
 * u niskopotrošni sleep mode radi uštede energije.
 */
...
#define buzzer 8      ///< Pin za aktivni buzzer (PWM pin nije potreban)
#define switchPin 2   ///< Pin za tipku/tipkalo za aktiviranje sleep moda (s PULLUP)
...
/**
 * @brief Zastavica za indikaciju buđenja iz sleep moda
...
/**
 * @brief Prekidna rutina za buđenje iz sleep moda
 * @details Poziva se kada se detektira FALLING edge na switchPin (tipka pritisnuta)
...
/**
 * @brief Ulazi u duboki sleep mode (PWR_DOWN)
 * @details Konfigurira najniži sleep mode gdje se zaustavlja sve osim watchdog timera
 *          i vanjskih prekida. Budi se samo preko prekida s tipke.
...
void enterSleep() {
  ...
  // Bezpieczne wejście w tryb uśpienia
  noInterrupts();
  interrupts(); 
  sleep_cpu();
 ...
 *          - Postavljanje prekidne rutine za tipku
 */
void setup() {
  Serial.begin(9600); // Inicijalizacija serijske komunikacije za debug
  ...
  pinMode(switchPin, INPUT_PULLUP); // Tipka s internim PULLUP otpornikom
  ...
  // Postavljanje prekidne rutine za tipku (FALLING edge - pritisak tipke)
 ...
 *          - Provjerava stanje tipke za sleep mode
 *          - Mjeri udaljenosti sa tri senzora
 ...
 */
void loop() {
  // Provjera tipke za sleep mode (HIGH = tipka otpuštena zbog PULLUP)
  ...
  if (minDist <= 10) {
    // Kriticna udaljenost - kontinuirani zvučni signal i zaustavljanje motora
    tone(buzzer, 2000); // Visok ton za hitnu situaciju
